...
Same-currency pairs (e.g. `EUR/EUR`) are intentionally not blocked. AlphaVantage returns a valid response for them, and there could be legitimate use cases for storing or querying them. Adding an explicit guard for this was left as a future decision rather than an assumption baked into the validation logic.
...
|---|---|---|
| `GET` | `/rates` | Returns all stored currency pairs exactly as they are in the database, with no staleness check and no provider call. Data may be arbitrarily old. |
...
- **GET /rates always returns potentially stale data:** The list-all endpoint reads directly from the database with no staleness check - it bypasses the resolver entirely. This is intentional for performance (calling a provider for every pair on every list request would be expensive), but callers have no way to know which rates are fresh and which are old. Two improvements to consider: (1) add an optional `?refresh=true` query parameter that triggers a bulk provider refresh before returning, so callers can explicitly request live data when needed; (2) expose `retrievedAtUtc` prominently so clients can decide client-side whether to trust each rate. The bulk refresh would require the provider to support a multi-pair request, or a coordinated burst of individual calls - neither of which AlphaVantage's free tier handles well, making this a good candidate for an async background job rather than a synchronous endpoint concern.
- **Concurrency on PUT / upsert (lost update problem):** Two clients reading the same rate and then both writing back will silently overwrite each other - the last writer wins with no error. A pessimistic lock (database row lock) would prevent this but does not scale under load. The right approach is **optimistic locking using EF Core's `RowVersion`**: add a `[Timestamp] byte[] RowVersion` property to `ExchangeRate`, which EF Core includes in the `WHERE` clause of every `UPDATE`. If the row changed since the client last read it, EF throws `DbUpdateConcurrencyException` and the middleware already maps that to HTTP 409 - the caller can then re-read the latest value and retry. The 409 handler exists but is currently dead code because no `RowVersion` is configured.
...
- **Smoke tests:** No smoke tests exist. A minimal set of real HTTP calls against a running instance should be added to verify the service is alive and the critical paths work after every deployment. The quickest approach is a dedicated test project using `WebApplicationFactory<Program>` (ASP.NET Core's in-process test server) - no external infrastructure needed, fast to run, and can be added to the CI pipeline as a post-deploy verification step. True smoke tests against a deployed URL (e.g. via a Reqnroll scenario or a Postman collection) can be added on top once a real deployment environment is available.
